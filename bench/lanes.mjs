 ...
 * under `bypassPermissions`. Read per call rather than captured at import. */
  ...
  try {
    for await (const m of query({
      ...
      options: {
        ...
        // file-tools arm actually runs. `bypassPermissions` would skip
        // `canUseTool` entirely, so the mode moves with it.
        ...(publicDemo()
