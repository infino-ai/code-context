...
describe("request shapes", () => {
  ...
  it("sub_agent posts only the fields given and decodes the facts", async () => {
    ...
    await db.subAgent({ question: "q", context: "Scope: src/superfile", mode: "explore", k: 10, projection: ["path", "start_line", "end_line"], max_turns: 3, max_wall_secs: 30, include_transcript: true });
    expect(bodyJson(calls[1])).toEqual({ question: "q", context: "Scope: src/superfile", mode: "explore", k: 10, projection: ["path", "start_line", "end_line"], max_turns: 3, max_wall_secs: 30, include_transcript: true });
