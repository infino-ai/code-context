...
Or zero-install, straight into Claude Code — one command, nothing else:
...
|---|---|---|
| `search` | One ranked pass fusing exact keyword matching (BM25) with semantic similarity (reciprocal-rank fusion). Hits carry the chunk content — answerable-from directly. | Finding code, always: exact identifiers AND paraphrases/renamed symbols in the same call. |
...
that's under a second, so search works before any embedding model even exists
...
storage — a live index the engine queries in place, not a snapshot you export
and pass around.
...
| `CX_NO_EMBED` | — | keyword-only mode for the MCP server (skip the vector stage) |
...
- **Index:** [Infino](https://github.com/infino-ai/infino) tables in
  `.infino/` — BM25 (FTS) + IVF vector indexes over one copy of the data,
...
- **Embeddings:** always local — a small model (chosen by a
  ...
  same model the index was built with — a mismatch is a clear error, not
